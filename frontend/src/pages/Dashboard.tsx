...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { v: "success" | "muted" | "danger"; l: string; dot: boolean }> = {
    completed: { v: "success", l: "Selesai", dot: false },
    running: { v: "muted", l: "Berjalan", dot: true },
    failed: { v: "danger", l: "Gagal", dot: false },
    cancelled: { v: "muted", l: "Dibatalkan", dot: false },
    pending: { v: "muted", l: "Antrian", dot: false },
  };
  const c = map[status] ?? { v: "muted" as const, l: status, dot: false };
  return <Badge variant={c.v} dot={c.dot}>{c.l}</Badge>;
}
  ...
  return (
    <div className="page-frame space-y-8">
      <div className="flex items-end justify-between gap-4 animate-fade-up">
        <div>
          <p className="page-kicker">arsip scan</p>
          <h1 className="section-title mt-2 text-[var(--text)]">Dasbor</h1>
          <p className="mt-2 text-sm text-[var(--text-secondary)]">Ringkasan deteksi duplikat konten YouTube</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="animate-fade-up stagger-1">
          <Stat label="Total Video" value={totalV} />
        </div>
        <div className="animate-fade-up stagger-2">
          <Stat label="Cluster Duplikat" value={totalC} />
        </div>
        <div className="animate-fade-up stagger-3">
          <Stat label="Video Duplikat" value={totalD} color="text-[var(--danger)]" />
        </div>
      </div>
      ...
      <Card className="animate-fade-up stagger-4 overflow-hidden">
        <div className="border-b border-[var(--border-strong)] bg-[var(--surface-muted)] px-5 py-4">
          <h2 className="text-sm font-black text-[var(--text)]">Riwayat</h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-20 text-center">
            ...
            <p className="text-sm font-bold text-[var(--text-secondary)]">Belum ada analisis</p>
            ...
            <table className="data-table w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border-strong)] text-left text-[11px] font-black uppercase tracking-wider text-[var(--text-secondary)]">
                  ...
                  <th className="px-4 py-3 text-right">Cluster</th>
                  <th className="px-4 py-3 text-right">Duplikat</th>
                  ...
                  <tr key={a.id} className={`group border-b border-[var(--border)] transition-colors animate-fade-up stagger-${Math.min(i + 5, 9)}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2 opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100">
                        <Link to={`/analyses/${a.id}`} className="rounded-md px-2 py-1 text-xs font-black text-[var(--primary)] hover:bg-[var(--primary-light)]">Lihat</Link>
                        {a.status === "running" ? (
                          <button onClick={() => handleCancel(a.id)} className="cursor-pointer rounded-md border-none bg-transparent px-2 py-1 text-xs font-black text-[var(--danger)] opacity-70 transition-opacity hover:bg-[var(--danger-light)] hover:opacity-100">Batal</button>
                        ) : (
                          <button onClick={() => handleDelete(a.id)} className="cursor-pointer rounded-md border-none bg-transparent px-2 py-1 text-xs font-black text-[var(--danger)] opacity-70 transition-opacity hover:bg-[var(--danger-light)] hover:opacity-100">Hapus</button>
                        )}
                      </div>
                    </td>
                  </tr>
