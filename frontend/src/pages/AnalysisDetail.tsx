...
function StatusBadge({ status }: { status: string }) {
  const { t } = useI18n();
  const map: Record<string, { key: string; variant: "success" | "danger" | "warning" | "default" }> = {
    completed: { key: "status.done", variant: "success" },
    running: { key: "status.running", variant: "warning" },
    failed: { key: "status.failed", variant: "danger" },
    cancelled: { key: "status.cancelled", variant: "default" },
    pending: { key: "status.pending", variant: "default" },
  };
  const cfg = map[status] ?? { key: status, variant: "default" as const };
...
function ProgressRing({ progress, message }: { progress: number; message: string }) {
  ...
  const stages = [
    { min: 0, max: 25, label: t("stage.downloadSubtitle") },
    { min: 25, max: 50, label: t("stage.audioTranscription") },
    { min: 50, max: 60, label: t("stage.parsePreprocess") },
    { min: 60, max: 90, label: t("stage.similarity") },
    { min: 90, max: 100, label: t("stage.report") },
  ];
...
export function AnalysisDetail() {
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      const [c, t] = await Promise.all([getClusters(analysisId), getTranscripts(analysisId)]);
      setClusters(c); setTranscripts(t);
    } catch {
  ...
  const handleCsvChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : t("detail.uploadFailed"));
    }
  };
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : t("detail.cancelFailed"));
    } finally {
      setCancelling(false);
    }
  };
    ...
    return (
      <div className="page-frame flex h-[60vh] items-center justify-center animate-fade-up">
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-[var(--text-secondary)]">{t("detail.notFound")}</p>
          <Link to="/" className="inline-flex rounded-md px-4 py-2 text-sm font-black action-secondary">{t("nav.dashboard")}</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="page-frame space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 animate-fade-up md:flex-row">
        <div>
          <div className="flex items-center gap-2.5">
            <Link to="/" className="mr-1 rounded-md px-2 py-1 text-sm font-black text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-muted)] hover:text-[var(--text)]">{t("detail.back")}</Link>
            <StatusBadge status={analysis.status} />
          </div>
          <p className="page-kicker mt-4">{t("detail.kicker")}</p>
        ...
        {isCompleted && (
          <div className="flex gap-2 animate-fade-up stagger-2">
            <label className="action-secondary relative flex cursor-pointer items-center gap-1.5 rounded-md px-3 py-2 text-xs font-black">
              ...
              {csvFile?.name || t("detail.uploadCsv")}
      ...
      {isRunning && (
        <div className="space-y-3">
          ...
          <button onClick={handleCancel} disabled={cancelling} className="flex items-center gap-1.5 rounded-md px-3 py-2 text-xs font-black text-[var(--danger)] border border-[var(--danger)] hover:bg-[var(--danger-light)] transition-colors disabled:opacity-50">
            ...
            {cancelling ? t("detail.cancelling") : t("detail.cancelBtn")}
          </button>
        </div>
      )}
      ...
      {isFailed && (
        <Card className="p-5 border-[#f1b9bd] animate-fade-up">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-black text-[var(--danger)]">{t("detail.failed")}</p>
              <p className="mt-0.5 text-xs text-[var(--text-secondary)]">{analysis.progress_message}</p>
              <Link to="/analyses/new" className="mt-3 inline-flex rounded-md px-3 py-1.5 text-xs font-black action-secondary">{t("detail.retry")}</Link>
            </div>
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 animate-fade-up stagger-2 lg:grid-cols-4">
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">{t("table.videos")}</p>
          <p className="mt-1 text-2xl font-black tabular-nums">{analysis.total_videos.toLocaleString("id-ID")}</p>
        </Card>
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">{t("table.clusters")}</p>
          <p className="mt-1 text-2xl font-black tabular-nums">{analysis.total_clusters}</p>
        </Card>
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">{t("table.duplicates")}</p>
          <p className="mt-1 text-2xl font-black tabular-nums text-[var(--danger)]">{analysis.total_duplicates}</p>
        </Card>
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">{t("detail.threshold")}</p>
        ...
        <div className="animate-fade-up stagger-3">
          <div className="mb-4 flex w-fit gap-1 rounded-md border border-[var(--border-strong)] bg-[var(--surface-muted)] p-1">
            <button onClick={() => setTab("clusters")} className={`rounded px-4 py-1.5 text-xs font-black transition-all ${tab === "clusters" ? "bg-[var(--surface)] text-[var(--text)] shadow-sm" : "text-[var(--text-secondary)] hover:text-[var(--text)]"}`}>
              {t("detail.tabClusters")} ({clusters.length})
            </button>
            <button onClick={() => setTab("transcripts")} className={`rounded px-4 py-1.5 text-xs font-black transition-all ${tab === "transcripts" ? "bg-[var(--surface)] text-[var(--text)] shadow-sm" : "text-[var(--text-secondary)] hover:text-[var(--text)]"}`}>
              {t("detail.tabTranscripts")} ({transcripts.length})
            </button>
          </div>
            ...
            <div className="space-y-4">
              {clusters.length === 0 ? (
                <Card className="p-16 text-center">
                  ...
                  <p className="text-sm font-black text-[var(--success)]">{t("detail.noDuplicates")}</p>
                  <p className="mt-1 text-xs text-[var(--text-secondary)]">
                    {t("detail.allUnique", { n: String(analysis.total_videos), t: analysis.threshold?.toFixed(2) ?? "0.75" })}
                  </p>
                </Card>
                ...
                <Card key={cluster.group_id} className={`overflow-hidden animate-fade-up stagger-${Math.min(ci + 4, 9)}`}>
                  <div className="flex flex-col gap-3 border-b border-[var(--border-strong)] bg-[var(--surface-muted)] px-5 py-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-black text-[var(--text)]">{t("detail.group")} {cluster.group_id}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-secondary)]">{cluster.cluster_size} {t("detail.videoCount")}</span>
                    ...
                    <table className="data-table w-full text-sm">
                      <thead>
                        <tr className="border-b border-[var(--border-strong)] text-[10px] font-black uppercase tracking-wider text-[var(--text-secondary)]">
                          <th className="px-5 py-2.5 text-left">{t("table.status")}</th>
                          <th className="px-4 py-2.5 text-left">{t("table.video")}</th>
                          <th className="px-4 py-2.5 text-right">{t("table.views")}</th>
                          <th className="px-4 py-2.5 text-right">{t("table.similarity")}</th>
                        </tr>
                      </thead>
                      <tbody>
                            ...
                            <tr className={`border-b border-[var(--border)] transition-colors ${
                              ...
                              <td className="px-5 py-2.5 align-top">
                                <Badge variant={v.status === "PRIMARY" ? "success" : "danger"}>
                                  {v.status === "PRIMARY" ? t("status.primary") : t("status.duplicate")}
                                </Badge>
                              </td>
            ...
            <Card className="overflow-hidden">
              <div className="table-shell">
                <table className="data-table w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--border-strong)] text-[10px] font-black uppercase tracking-wider text-[var(--text-secondary)]">
                      <th className="px-5 py-2.5 text-left">{t("transcript.videoId")}</th>
                      <th className="px-4 py-2.5 text-left">{t("transcript.preview")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {transcripts.map((t) => (
