...
function ProgressRing({ progress, message }: { progress: number; message: string }) {
  ...
  const stages = [
    { min: 0, max: 30, label: "Download Subtitle" },
    { min: 30, max: 55, label: "Parse & Preprocess" },
    { min: 55, max: 85, label: "Similarity Analysis" },
    { min: 85, max: 100, label: "Generate Report" },
  ];
    ...
    return (
      <div className="page-frame flex h-[60vh] items-center justify-center animate-fade-up">
        <div className="text-center space-y-4">
          ...
          <Link to="/" className="inline-flex rounded-md px-4 py-2 text-sm font-black action-secondary">Dashboard</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="page-frame space-y-6">
      <div className="flex flex-col items-start justify-between gap-4 animate-fade-up md:flex-row">
        <div>
          <div className="flex items-center gap-2.5">
            <Link to="/" className="mr-1 rounded-md px-2 py-1 text-sm font-black text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-muted)] hover:text-[var(--text)]">Back</Link>
            <StatusBadge status={analysis.status} />
          </div>
          <p className="page-kicker mt-4">analysis detail</p>
      ...
      {isFailed && (
        <Card className="p-5 border-[#f1b9bd] animate-fade-up">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-black text-[var(--danger)]">Analysis failed</p>
              <p className="mt-0.5 text-xs text-[var(--text-secondary)]">{analysis.progress_message}</p>
              <Link to="/analyses/new" className="mt-3 inline-flex rounded-md px-3 py-1.5 text-xs font-black action-secondary">Retry</Link>
            </div>
          </div>
        </Card>
      )}
      ...
      <div className="grid grid-cols-2 gap-3 animate-fade-up stagger-2 lg:grid-cols-4">
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">Videos</p>
          <p className="mt-1 text-2xl font-black tabular-nums">{analysis.total_videos.toLocaleString("id-ID")}</p>
        </Card>
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">Clusters</p>
          <p className="mt-1 text-2xl font-black tabular-nums">{analysis.total_clusters}</p>
        </Card>
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">Duplicates</p>
          <p className="mt-1 text-2xl font-black tabular-nums text-[var(--danger)]">{analysis.total_duplicates}</p>
        </Card>
        <Card className="p-4 text-center transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-hover)]">
          <p className="text-[10px] uppercase tracking-wider text-[var(--text-secondary)] font-black">Threshold</p>
            ...
            <div className="space-y-4">
              {clusters.length === 0 ? (
                <Card className="p-16 text-center">
                  ...
                  <p className="text-sm font-black text-[var(--success)]">No duplicates found</p>
                  <p className="mt-1 text-xs text-[var(--text-secondary)]">
                    All {analysis.total_videos} videos are unique (threshold {analysis.threshold?.toFixed(2)})
                  </p>
                </Card>
                ...
                <Card key={cluster.group_id} className={`overflow-hidden animate-fade-up stagger-${Math.min(ci + 4, 9)}`}>
                  <div className="flex flex-col gap-3 border-b border-[var(--border-strong)] bg-[var(--surface-muted)] px-5 py-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-black text-[var(--text)]">Group {cluster.group_id}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] font-black uppercase tracking-wider text-[var(--text-secondary)]">{cluster.cluster_size} videos</span>
            ...
            <Card className="overflow-hidden">
              <div className="table-shell">
                <table className="data-table w-full text-sm">
                  <thead>
                    <tr className="border-b border-[var(--border-strong)] text-[10px] font-black uppercase tracking-wider text-[var(--text-secondary)]">
                      <th className="px-5 py-2.5 text-left">Video ID</th>
